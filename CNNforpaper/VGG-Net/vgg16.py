...
class vgg16:
    def __init__(self, imgs, weight=None, sess=None):
        self.imgs = imgs
        self.convlayers()
        self.fc_layers()
        self.prob = tf.nn.softmax(self.fc3)
    ...
    def fc_layers(self):
        ...
        with tf.name_scope('fc3') as scope:
            fc3w = tf.Variable(tf.truncated_normal([4096, 4096], dtype=tf.float32, stddev=1e-1), name='weights')
            fc3b = tf.Variable(tf.constant(1.0, shape=[4096], dtype=tf.float32), trainable=True, name='biases')
            fc3l = tf.nn.bias_add(tf.matmul(self.fc3, fc3w), fc3b)
...
if __name__ == '__main__':
    ...
    # prob = sess.run(vgg.prob, feed_dict=(vgg.imgs: {img1}))[0]
    # preds = (np.argsort(prob)[::-1])[0: 5]
    # for p in preds:
    #     print(class_name[p], prob)